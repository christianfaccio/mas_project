...
species spectator skills:[moving] control: simple_bdi {
    ...
    // Perceptions ---------------------------------------------------------------------------------------
   	...
   	perceive target: worker in: perception_distance {
	    ask myself {
	        if not has_belief(alerted) {
	...
	perceive target: spectator where (each.has_belief(alerted)) in: perception_distance {
	    ask myself {
	        if not has_belief(alerted) {
	            do remove_belief(not_alerted);
	            do add_belief(alerted);
    ...
    plan watching intention: watch {
		// Just watching - do nothing
    }
    
    plan escape_danger intention: escape finished_when: (location distance_to safety_point) < 2#m {
